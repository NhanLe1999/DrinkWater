...
public class MetaballParticleClass : MonoBehaviour
{
    ...
    void Update()
    {
       // Debug.Log("hiii_scale: " + transform.localScale);
        ...
        if (Active == true)
        {
            ...
            {
                if (gameObject.transform.position.y <= LogicGame.Instance.TrsObj.position.y || gameObject.transform.position.y >= LogicGame.Instance.TrsObjUp.position.y)
                {
                    delta *= 0;
                    Active = false;
                    SpawnerParent.DropsUsed--;
